...
-- name: DeletePersonnePhysique :exec
DELETE
FROM t_pers_physiques
WHERE id_pers_physique = $1;

-- name: SubmitDraftPersonnePhysique :exec
UPDATE t_pers_physiques
...
VALUES (sqlc.arg(prenom_nom_pers_phy),
        ...
        sqlc.arg(parent_id),
        sqlc.arg(user_id),
        sqlc.arg(historiographie),
        sqlc.arg(evenements),
        sqlc.arg(preparatifs),
        sqlc.arg(chemin_suivi),
        sqlc.arg(arrivee),
        sqlc.arg(retour),
        sqlc.arg(non_execution),
        sqlc.arg(age),
        sqlc.arg(composition_groupe))
...
-- name: LinkPersPhyToPersPhy :exec
INSERT INTO cor_pers_phy_pers_phy
    (pers_physique_id_1, pers_physique_id_2)
SELECT sqlc.arg(id), unnest(sqlc.arg(pers_phy_ids)::int[]);

-- name: UnlinkPersPhyFromPersPhy :exec
DELETE
FROM cor_pers_phy_pers_phy
WHERE pers_physique_id_1 = sqlc.arg(id)
   OR pers_physique_id_2 = sqlc.arg(id);