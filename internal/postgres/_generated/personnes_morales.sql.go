...
const getDraftPersonnesMorales = `-- name: GetDraftPersonnesMorales :many
...
type GetDraftPersonnesMoralesRow struct {
	ID                      int32
	Title                   pgtype.Text
...
func (q *Queries) GetDraftPersonnesMorales(ctx context.Context, userID pgtype.Text) ([]GetDraftPersonnesMoralesRow, error) {
	rows, err := q.db.Query(ctx, getDraftPersonnesMorales, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetDraftPersonnesMoralesRow
	for rows.Next() {
		var i GetDraftPersonnesMoralesRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.FoundationDeed,
