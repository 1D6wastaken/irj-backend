...
const getDraftMobiliersImages = `-- name: GetDraftMobiliersImages :many
...
type GetDraftMobiliersImagesRow struct {
	ID                      int32
	Title                   string
	Description             pgtype.Text
	Historique              pgtype.Text
...
func (q *Queries) GetDraftMobiliersImages(ctx context.Context, userID pgtype.Text) ([]GetDraftMobiliersImagesRow, error) {
	rows, err := q.db.Query(ctx, getDraftMobiliersImages, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetDraftMobiliersImagesRow
	for rows.Next() {
		var i GetDraftMobiliersImagesRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Description,
