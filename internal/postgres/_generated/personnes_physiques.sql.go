...
const deletePersonnePhysique = `-- name: DeletePersonnePhysique :exec
DELETE
FROM t_pers_physiques
WHERE id_pers_physique = $1
`

func (q *Queries) DeletePersonnePhysique(ctx context.Context, idPersPhysique int32) error {
	_, err := q.db.Exec(ctx, deletePersonnePhysique, idPersPhysique)
	return err
}
...
const getDraftPersonnesPhysiques = `-- name: GetDraftPersonnesPhysiques :many
...
func (q *Queries) GetDraftPersonnesPhysiques(ctx context.Context, userID pgtype.Text) ([]GetDraftPersonnesPhysiquesRow, error) {
	rows, err := q.db.Query(ctx, getDraftPersonnesPhysiques, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetDraftPersonnesPhysiquesRow
	for rows.Next() {
		var i GetDraftPersonnesPhysiquesRow
		if err := rows.Scan(
			&i.ID,
			&i.Firstname,
			&i.DateNaissance,
