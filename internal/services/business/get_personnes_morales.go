...
func (b *BusinessService) GetDraftPersonnesMorales(w http.ResponseWriter, r *http.Request) *_http.APIError {
	ctx, cancel := context.WithTimeout(r.Context(), defaultTimeOut)
	defer cancel()
	...
	persMo, err := b.postgresService.Queries.GetDraftPersonnesMorales(ctx, pgtype.Text{
		String: token.ID,
		Valid:  true,
	})
	if err != nil {
		logger.Error().Err(err).Msg("failed to get draft personnes morales")
...
func (b *BusinessService) GetPersonneMorale(w http.ResponseWriter, r *http.Request) *_http.APIError {
	...
	return _http.WriteJSONResponse(w, http.StatusOK, api.PersonneMorale{
		...
		RefondationDate:       pmo.RefondationDate.String,
		DateFin:               pmo.DateFin.String,
		StatusText:            pmo.TexteStatuts.Bool,
		OrigineSocialeProf:    pmo.OrigineSocialeProf.String,
		MembresConnus:         pmo.MembresConnus.String,
		FrequenceReunions:     pmo.FrequenceReunions.String,
		ParticipationViePol:   pmo.ParticipationViePol.String,
		Funerailles:           pmo.Funerailles.String,
		AutresFetes:           pmo.AutresFetes.String,
		InhumationCostume:     pmo.InhumationCostume.String,
		Fondateurs:            pmo.Fondateurs.String,
		Statuts:               pmo.Statuts.String,
		Autorisations:         pmo.Autorisations.String,
	})
}
