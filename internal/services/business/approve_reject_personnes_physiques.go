...
func sendApprovalEmailPersPhy(_ context.Context, s *BusinessService, exData *approveRejectPersonnePhysiqueExchangeData) approveRejectPersonnePhysiqueState {
	...
	go func(logger *zerolog.Logger, userID string, id int32, title string, isUpdate bool) {
		...
		if err := s.smtpService.SendDocumentApprovedMail(ctx, to, smtp.SourcePersonnesPhysiques, id, isUpdate, title); err != nil {
			logger.Error().Err(err).Msg("failed to send approval email")
		}
	}(exData.logger, exData.documents.UserID.String, exData.id, exData.documents.Firstname.String, exData.documents.ParentID.Valid)
